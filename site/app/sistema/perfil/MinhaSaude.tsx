...
import { inputCls, Section, Field, G2, G3, TabLoader, SaveBtn, Feedback } from '../cadastros/funcionarios/components/tabs/_shared'
...
function FileUploadBtn({
  label, onUploaded, usuarioId, tipo,
}: {
  label: string; onUploaded: (url: string) => void; usuarioId: string; tipo: string
}) {
  ...
  async function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      fd.append('file', file); fd.append('usuarioId', usuarioId); fd.append('tipo', tipo)
...
export function MinhaSaude({ usuarioId }: { usuarioId: string }) {
  ...
  useEffect(() => {
    Promise.all([getFuncSaude(usuarioId), getFuncAtestados(usuarioId)]).then(([saude, atts]) => {
      if (saude) {
        setForm({
  ...
  async function handleAddAtestado() {
    ...
    } catch (err) { setAttError(err instanceof Error ? err.message : 'Erro ao registrar') }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <Section title="Atestados de Saúde Ocupacional (ASO)">
        <G3>
          {(['admissional', 'periodico', 'retorno'] as const).map((tipo) => {
            ...
            const labels  = { admissional: 'ASO Admissional', periodico: 'ASO Periódico', retorno: 'ASO Retorno' }
            return (
              <Field key={tipo} label={labels[tipo]}>
                <input type="date" className={inputCls} value={form[dataKey]}
                ...
                <div className="mt-1.5 flex items-center gap-2">
                  {form[urlKey] ? (
                    ...
                    <FileUploadBtn label="Anexar PDF" tipo={`aso-${tipo}`} usuarioId={usuarioId}
      ...
      {/* Atestados médicos */}
      <Section title="Meus Atestados Médicos">
        {atestados.length > 0 && (
                ...
                {atestados.map((a) => (
                  <tr key={a.id}>
                    ...
                    <td className="px-2 py-3">
                      <button type="button" onClick={() => removeFuncAtestado(a.id).then(loadAtestados)}
        ...
        <G2>
          ...
          <Field label="Descrição">
            <input type="text" className={inputCls} placeholder="Médico, CID, observações..."
        ...
        <div className="mt-2 flex items-center gap-4">
          {attArquivo ? (
            ...
            <FileUploadBtn label="Anexar atestado" tipo="atestado" usuarioId={usuarioId} onUploaded={setAttArquivo} />
          )}
        </div>
        {attError && <p className="text-red-500 text-sm mt-1">{attError}</p>}
        <button type="button" disabled={addingAtt || !newAtt.data} onClick={handleAddAtestado}
          ...
          {addingAtt ? <span className="w-3.5 h-3.5 border-2 border-gold border-t-transparent rounded-full animate-spin" /> : (
